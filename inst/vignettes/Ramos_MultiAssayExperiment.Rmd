...
.sysLink <- function(...) {
    system.file(file.path("vignettes", ...),
        package = "MultiAssayWorkshop")
}
```
...
using the `MultiAssayExperiment` data class and methods.  It introduces the
facilities made available by companion packages, `cBioPortalData`,
`curatedTCGAData`, and `TCGAutils` to work with publicly available multi-omic
data. Using the `MultiAssayExperiment` class, these packages make the
management of multiple assays easier and more efficient. The workshop also
covers, to a limited extent, relevant data classes such as
`SummarizedExperiment` and `RangedSummarizedExperiment`, which provide
efficient and powerful operations for representation of copy number, mutation,
variant, and expression data that are represented by different genomic ranges
for each specimen.
...
by the `curatedTCGAData` and `cBioPortalData` packages.

### R/Bioconductor packages used

First, please make sure you have the latest Bioconductor release version 3.10.
...
## Bioconductor 3.10 Stable Release
...
### cBioPortalData installation from GitHub
...
coordination of multi-omic public data

### Learning objectives

* use `curatedTCGAData` and `cBioPortalData` to create on-the-fly multi-omics
data objects
* create a `MultiAssayExperiment` for other multi'omics data
...
knitr::include_graphics(.sysLink("MultiAssayExperiment.png"))
```
...
To see what studies are available from the `cBioPortal`, use `getStudies`
on the `cbio` API object:

```{r}
knitr::kable(getStudies(cbio), align = "l", escape = FALSE,
    caption = "List of available studies from the cBioPortal") %>%
...
knitr::read_chunk(.sysLink("dataTypesHTML.R"))
```
...
```{r, echo=FALSE, cache=FALSE, message=FALSE, warning=FALSE}
knitr::read_chunk(.sysLink("TCGAutils_cheatHTML.R"))
```
...
knitr::include_graphics(.sysLink("TCGAMAEPipeline20190204.png"))
```
...
knitr::include_graphics(.sysLink("SummarizedExperiment.png"))
```
...
knitr::include_graphics(.sysLink("RaggedExperiment.png"))
```
...
[MultiAssayExperiment issue tracker](https://github.com/waldronlab/MultiAssayExperiment/issues).
...
knitr::include_graphics(.sysLink("MultiAssayExperiment_cheatsheet.png"))
```
